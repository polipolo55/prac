 ...
 * Programa modular para la encriptación de mensajes mediante metodos de substitución o permutación 
 * usando las clases <em>Alfabets<em>, <em>Encriptacions<em>, y <em>Missatges<em>
*/

/**
 * @file program.cc
 * @author Pol Casanovas Puig (pol.casanovas.puig@estudiantat.upc.edu)
 * @brief La parte principal de programa con la gestión de la input de los comandos
 * @version 0.1 
 * @date 07-11-22
 * 
 */
  ...
  while (s != "fin") {
    if (s == "nuevo_mensaje" or s == "nm") {
      ...
      if (!alf.is_alf_there(ida)) {
        alf.add_alf(ida);
        cout << alf.n_alf() << endl;
      } else cout << "ja existeix l'alfabet: " << ida << endl;
      ...
      if (msg.is_msg_there(idm)) {
        msg.del_msg(idm);
        cout << msg.n_msg() << endl;
      } else cout << "el missatge " << idm << " no existeix" << endl;
      ...
      if (alf.is_alf_there(ida)) {
        if (1 > alf.n_of_msg(ida)){
          alf.del_alf(ida);
        } else cout << "l'alfabet " << ida << " conté missatges" << endl; 
      } else cout << "no existeix l'alfabet " << ida << endl; 
      ...
      } else cout << "l'alfabet " << ida << " no existeix" << endl; 
